...
description: 알약꾹 백엔드 기능 하나의 실행 가능한 SDD Feature Packet을 로컬 요구사항 기준에서 생성하거나 갱신한다. 기능 정의·구체화·분리·승인, 인수 조건 추가, 데이터·ERD 영향 정리 또는 구현 전 범위 확정을 요청받았을 때 사용한다. 외부 문서 서비스 연결을 요구하지 않으며 사용자가 구현을 별도로 요청하지 않으면 운영 코드를 구현하지 않는다.
---
