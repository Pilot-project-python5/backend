...
- 적용 범위: 활성 카테고리 11개, 게시 추천 제품 32개, 제품별 코치 코멘트·쿠팡 링크·사진
...
- 원본에 가격이 없으므로 `display_price=0`을 “가격 미제공” 값으로 사용한다. 실시간
  판매가를 추정하거나 수집하지 않는다.
- 원본에서 확인한 제품별 사진 32개는 최대 1200px WebP로 정규화하고 EXIF를 포함한
  메타데이터를 제거해 `/static/products/{sku-lowercase}.webp`에 보관한다. 시드와
  프론트엔드는 실행 시 원본 이미지 호스트에 의존하지 않는다.
