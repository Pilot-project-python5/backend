...
3. features/manifest.yaml에서 기능 ID, 단계와 담당 경계를 확인한다.
4. 작업 대상의 Feature Packet을 읽는다.
5. 데이터 변경이 있으면 architecture/erd.md와 data-model.md를 읽는다.
6. architecture와 api 문서로 나머지 설계 제약을 확인한다.
7. development 문서의 로컬 실행과 검증 절차를 따른다.
8. 구현 완료 기능을 조사할 때 history의 기능 이력 문서를 읽는다.
9. 출처 이력이 필요할 때만 source-map.md를 확인한다.
