...
가격·로컬 제품 이미지와 정확한 total·has_next를 제공한다. UI 시안의 제품 3종과
...
- 제품 3종·카테고리 매핑 멱등 시드와 실제 로컬 SVG 정적 이미지
...
- `(is_published, sort_order, sku)`와 `(category_id, product_id)` 조회 인덱스를
  추가했다.
- 제품 3종을 SKU 자연 키로 시드하고 vitamin·protein·omega-3 승인 매핑을 복원한다.
...
- 가격은 실시간 판매가가 아닌 로컬 시드 참고 가격이다.
- 로컬 SVG는 개발·연동용 이미지이며 운영용 상품 원본이나 CDN 최적화를 제공하지 않는다.
