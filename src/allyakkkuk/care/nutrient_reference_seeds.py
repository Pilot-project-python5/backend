...
from allyakkkuk.care.nutrient_reference_csv import load_reference_csv
...
class NutrientReferenceSeedSet:
    ...
    def apply(self, connection: Connection) -> int:
        ...
        version_id = uuid5(
            NAMESPACE_URL, f"allyakkkuk:nutrient-reference:{document.version}"
        )
                ...
                [
                    {
                        "id": uuid5(
                            NAMESPACE_URL,
                            f"allyakkkuk:{document.version}:{row.nutrient_code}:{row.gender}:{row.age_min}:{row.age_max}:{row.reference_type}",
                        ),
